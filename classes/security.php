...
class Security
{
	...
	public static function getUserGroups()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "Label" )
			." from ". $grConnection->addTableWrappers( "gestionentidad_v108uggroups" ) . " WHERE " . $grConnection->addFieldWrappers( "GroupID" )
	...
	*  For AD it is <provider code><username>
	...
	public static function getUgMembersDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "gestionentidad_v108ugmembers", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgGroupsDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "gestionentidad_v108uggroups", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgRightsDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "gestionentidad_v108ugrights", $cman->getUserGroupsConnId() );
	}
	...
	public static function  fillTablesOwnerId( $data, $provider ) {
		...
		if( $securityType == stAD && !$provider["useDbGroups"] ) {
			...
			storageSet( "_dependencia_SJ_OwnerID", $userId );
		} else {
						storageSet( "OwnerID", $data["usr_directivo"] );
			storageSet( "_dependencia_SJ_OwnerID", $data["usr_directivo"] );
		}
	}
